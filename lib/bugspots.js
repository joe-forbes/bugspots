...
Bugspots.prototype.scan = function (opts, callback, commitInclusionDecisionHandler) {
  ...
  var commitListHandler = function (err, commits) {
    ...
    var commitsProcessed = [];

    commits.forEach(function (commit) {
        commitsProcessed.push(commit.id);
        ...
        exec(cmd, opt, function execFn(err, stdout) {
          ...
          if (!markerCommitFound) {
            if (0 === commit.id.indexOf(markerCommitId)) {
              markerCommit = commit;
            }
          }
          ...
          if (commitsInProgress <= 0) {
            if (depthAchieved === depth) {
              ...
              repo.commits(headCommit.id, depthToRetrieve, batchesReceived * batchSize, commitListHandler)
            }
          }
        })
      }
  ...
  var headCommitHandler = function (err, head) {
    ...
    logger.debug('getting commits: ' + JSON.stringify({commitId: headCommit.id, depthToRetrieve: depthToRetrieve}));
    repo.commits(headCommit.id, depthToRetrieve, 0, commitListHandler);
  };
