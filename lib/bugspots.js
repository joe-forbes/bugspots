...
Bugspots.prototype.scan = function (opts, callback, commitInclusionDecisionHandler) {
  ...
  var commitListHandler = function (err, commits) {
    ...
    if (commits.length < 1) {
      logger.debug('bottom of commit history reached. calling sendOutput()');
      sendOutput();
    }
...
//            if (depthAchieved === depth) {
            if (0 === depthRemaining) {
              logger.debug('depth achieved. calling sendOutput()');
              sendOutput();
            } else {
              ...
              repo.commits(branch, depthToRetrieve, batchesReceived * batchSize, commitListHandler)
            }
          }
        }
  ...
  logger.debug('getting commits: ' + JSON.stringify({commitId: branch, depthToRetrieve: depthToRetrieve}));
  repo.commits(branch, depthToRetrieve, 0, commitListHandler);
